...
from evals.helpers import ScriptedClient, make_milli, response, text_block, tool_block
...
def _tmp_workspace(tmp_path, monkeypatch):
    """Never let a delegate test write into the repo's ./milli_workspace."""
    monkeypatch.setenv("MILLI_WORKSPACE", str(tmp_path / "ws"))
...
def test_delegate_task_invokes_pi_print_mode(tmp_path, monkeypatch):
    ...
    record = {}
    monkeypatch.setenv("MILLI_EXPERIMENTAL", "1")
    monkeypatch.setenv("MILLI_WORKSPACE", str(tmp_path / "ws"))   # keep it out of the repo
    ...
    app = make_milli(tmp_path / "home", client=ScriptedClient(script))
...
def test_delegate_timeout_is_honest(tmp_path, monkeypatch):
    ...
    assert "7s" in out and "MILLI_DELEGATE_TIMEOUT" in out
...
def test_delegate_kills_a_silent_pi_at_the_deadline(tmp_path, monkeypatch):
    ...
    assert "2s" in out and "MILLI_DELEGATE_TIMEOUT" in out
...
def test_experimental_flag_gates_registration(tmp_path, monkeypatch):
    ...
    monkeypatch.delenv("MILLI_EXPERIMENTAL", raising=False)
    app_off = make_milli(tmp_path / "off", client=ScriptedClient([]))
    assert "delegate_task" not in app_off.tools._tools

    monkeypatch.setenv("MILLI_EXPERIMENTAL", "1")
    app_on = make_milli(tmp_path / "on", client=ScriptedClient([]))
