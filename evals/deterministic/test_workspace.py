"""DETERMINISTIC EVAL — the delegated-coding workspace (milli.tools.workspace).
...
from milli.tools import workspace as ws


def test_run_folder_is_dated_and_named(tmp_path, monkeypatch):
    monkeypatch.setenv("MILLI_WORKSPACE", str(tmp_path))
...
def test_autorun_disabled_by_env(tmp_path, monkeypatch):
    monkeypatch.setenv("MILLI_WORKSPACE", str(tmp_path))
    monkeypatch.setenv("MILLI_DELEGATE_AUTORUN", "0")
