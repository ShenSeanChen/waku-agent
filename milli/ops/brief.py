"""`python -m milli brief` — a morning briefing that runs through the normal
...
def main() -> None:
    console = Console()
    milli = Milli()
    if not milli.settings.apple_tools:
        console.print("[dim]Tip: set MILLI_APPLE_TOOLS=1 to brief from your real Calendar and Mail.[/dim]")
    result = milli.respond(PROMPT, source="brief")
    console.print(result.reply)
    out = milli.settings.home / "outbox" / f"brief-{date.today().isoformat()}.txt"
