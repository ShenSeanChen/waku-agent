...
aligned rows each, then the milli punchline. Facts are sourced on the board
...
Run:  python -m milli.ops.whiteboard.build_pi_vs_claude
"""
...
from milli.ops.whiteboard import style as S
...
ROWS = [
    ...
    ("SUB-AGENTS",
     ...
     "refused — spawn pi yourself: tmux,\nor milli's delegate_task (that's us)",
     "plain"),
    ("MCP",
...
def build() -> list:
    ...
    e += S.labeled_box(80, band, 2410, 96,
                       "milli uses BOTH: Claude Code writes milli's code - pi is milli's delegate_task sub-agent,\n"
