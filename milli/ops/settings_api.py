...
from milli.ops import browser_agent, catalog
...
def settings_info() -> dict:
    ...
    from milli.loop.models import PROVIDERS
    ...
    return {
        ...
        "pinned": pinned,
        # a custom endpoint (e.g. OpenRouter) set via MILLI_BASE_URL / MILLI_API_KEY
...
def apply_settings(payload: dict) -> dict:
    ...
    before = {"provider": os.getenv("MILLI_PROVIDER", ""),
              "model": os.getenv("MILLI_MODEL", ""),
              "small_model": os.getenv("MILLI_SMALL_MODEL", "")}
    writable = ({"MILLI_PROVIDER", "MILLI_MODEL", "MILLI_SMALL_MODEL", "TAVILY_API_KEY",
                 "MILLI_EPISODIC_STORE", "MILLI_EXPERIMENTAL", "MILLI_GRAPH_WORKFLOWS",
                 "NOTION_TOKEN", "NOTION_EPISODES_DATABASE_ID"}
                | {p.key_env for p in PROVIDERS.values()})
    env_path = find_dotenv(usecwd=True) or ".env"

    updates = {"MILLI_PROVIDER": provider,
               "MILLI_MODEL": payload.get("model", "") or "",
               "MILLI_SMALL_MODEL": payload.get("small_model", "") or ""}
    if episodic_store:
        updates["MILLI_EPISODIC_STORE"] = episodic_store
    ...
    if experimental is not None:
        updates["MILLI_EXPERIMENTAL"] = "1" if str(experimental).strip() else ""
    graph_workflows = payload.get("graph_workflows")  # same is-not-None rule
    if graph_workflows is not None:
        updates["MILLI_GRAPH_WORKFLOWS"] = "1" if str(graph_workflows).strip() else ""
    ...
    if provider != before["provider"]:
        if updates["MILLI_MODEL"] in ("", before["model"]):
            updates["MILLI_MODEL"] = catalog.default_model_for(provider)
        if updates["MILLI_SMALL_MODEL"] in ("", before["small_model"]):
            updates["MILLI_SMALL_MODEL"] = ""
    for k, v in (payload.get("keys") or {}).items():
        if k in writable and v:  # only non-empty keys overwrite
            if k == "NOTION_EPISODES_DATABASE_ID":
                from milli.memory.episodic.notion_store import normalize_database_id
    ...
    browser_agent.current().tracer.event("config", {
        "from": before,
        "to": {"provider": provider, "model": updates["MILLI_MODEL"],
               "small_model": updates["MILLI_SMALL_MODEL"]},
    })
    return {"ok": True, **settings_info()}
