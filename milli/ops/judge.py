...
Completion (milli.ops.scoring) is deterministic — did the right tool fire. Quality
...
pure text, no tools). Switchable per-race from the arena, or via MILLI_JUDGE_*.
Any provider works — Milli's OpenAI-compat client exposes the same
...
from milli.loop.models import get_client

JUDGE_PROVIDER = os.getenv("MILLI_JUDGE_PROVIDER", "openai")
JUDGE_MODEL = os.getenv("MILLI_JUDGE_MODEL", "gpt-5.6-sol")
...
_JUDGE_SEM = threading.Semaphore(int(os.getenv("MILLI_JUDGE_CONCURRENCY", "2")))
