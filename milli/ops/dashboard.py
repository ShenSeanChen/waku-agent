...
One stdlib HTTP server reading the files Milli already writes:
...
from milli.ops import browser_agent, commands, compare_history
from milli.ops.arena import (
    compare_clear,
    compare_delete_run,
    compare_regrade,
    compare_stream,
...
from milli.ops.browser_agent import agent_lock, dash_session, get_agent, maybe_rotate_session
from milli.ops.catalog import list_models
from milli.ops.pricing import price_for, usage_summary
from milli.ops.settings_api import apply_settings, pin_action, settings_info
from milli.ops.tracing import TraceEncodingError, iter_trace_lines
...
def _get_notion_store():
    ...
    if _notion_store is None:
        from milli.memory.episodic.notion_store import NotionEpisodeStore
...
def collect() -> dict:
    ...
    from milli.memory import bundled_skill_dirs
    from milli.memory.procedural.loader import SkillLoader
    ...
    from milli.graph.workflows.gather import gather_topology
    from milli.graph.workflows.triage import triage_topology
    ...
    return {
        ...
        "graph": {
            ...
            # `milli gather` is a routine you run yourself and ignores this flag
...
def _rel_to_home(path, home) -> str:
    """Path relative to MILLI_HOME if it lives there, else the repo-relative
...
def session_list(conn) -> list[dict]:
    ...
    for g in groups:
        ...
        preview = ""
        if last:
            preview = ("you: " if last["role"] == "user" else "milli: ") + last["content"][:80]
        out.append({"id": sid,
...
def tools_info() -> dict:
    ...
    if live is not None:
        ...
        from milli.tools import calendar, memory_admin, messages, notes, search
        ...
        if settings.experimental:
            ...
            # switch did nothing.
            from milli.tools import experimental as experimental_tools
    ...
    from milli.tools.experimental import PLANNED
...
def transcribe_audio(raw: bytes) -> dict:
    ...
    with _whisper_lock:
        if _whisper is None:
            _whisper = WhisperModel(os.getenv("MILLI_WHISPER_MODEL", "base"), compute_type="int8")
...
def _editor_cmd() -> list[str] | None:
    """The user's code editor CLI: $MILLI_EDITOR, then cursor, then code."""

    custom = os.getenv("MILLI_EDITOR")
...
def reveal_path(rel: str) -> dict:
    """Open a file/folder under MILLI_HOME — in the user's code editor if one
    is on PATH (cursor/code/$MILLI_EDITOR), otherwise reveal in Finder.
    Restricted to paths inside MILLI_HOME."""
    import subprocess
    import sys
    ...
    if target != home and home not in target.parents:
        return {"error": "path is outside the .milli home"}
...
def memory_action(payload: dict) -> dict:
    ...
    if action == "save_skill":
        ...
        from milli.memory import bundled_skill_dirs
        from milli.memory.procedural.loader import _parse_text
...
def main() -> None:
    # Port precedence: MILLI_DASHBOARD_PORT, then the conventional PORT (used by
    # deploy platforms and IDE preview panes), then 7777. If it's taken, walk on.
    base = int(os.getenv("MILLI_DASHBOARD_PORT") or os.getenv("PORT") or PORT)
    for port in range(base, base + 10):  # walk past a busy port instead of crashing
        ...
        # too (background thread) so you don't need a separate `milli telegram`.
        try:
            from milli.gateway.telegram import start_in_background
        ...
        try:
            from milli.gateway.discord import start_in_background as start_discord
        ...
        try:
            from milli.gateway.whatsapp import start_in_background as wa_background

            if wa_background():
                print("WhatsApp gateway → listening in the background (webhook on port 5000)")
        except Exception as exc:  # noqa: BLE001 — never let a gateway block the dashboard
            print(f"(whatsapp) not started: {exc}")
        print(f"Milli dashboard → http://localhost:{port}  (Ctrl-C to stop)")
