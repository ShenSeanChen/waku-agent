// milli dashboard — subtab/db helpers, SQL console, Memory/Tools sub-views, VIEWS.
...
function memOverview(d){
  ...
  return `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
      ...
      <div class="r">This tab is the curated, per-pillar view of what Milli remembers. The
      ...
      <code>.milli/state.db</code> — different altitude.
      <br><br>Some assistants (Hermes) keep memory as a single <code>MEMORY.md</code> file. Milli keeps
...
function memSemantic(d){
  let h = `<div class="meta" style="margin-bottom:12px">Durable facts distilled from what you tell Milli —
...
function memSkills(d){
  let h = `<div class="meta" style="margin-bottom:12px">Procedural memory — markdown instructions loaded
    only when a message matches. Add your own three ways: teach Milli in chat (it calls
...
function memSoul(d){
  return `<div class="meta" style="margin-bottom:12px">SOUL.md is Milli's persona — the system prompt it
    loads every turn. Editing it changes who your Milli is. Changes are live next turn.</div>
...
function toolsMCP(t){
  ...
  let h = `<div class="card ${m.configured?"":""}" style="border-color:${m.live?"var(--good)":"var(--line2)"}">
    ...
    <div class="r">MCP lets Milli borrow tools from any external server (files, GitHub, a database, …),
  ...
  h += `<h2>Connect one (30 seconds)</h2><div class="card">
    ...
    <div class="meta" style="margin-top:6px">2 — create ${reveal("","the .milli folder")}<code>/mcp.json</code>:</div>
  ...
  h += `<div class="meta" style="margin-top:12px">The same pattern scales: any MCP server (yours or a vendor's)
    plugs in the same way — no code changes to Milli. Skills work the same way — drop a <code>SKILL.md</code>
...
const VIEWS = {
  ...
  overview(d){
    ...
    ${graphPanel(d)}
    <h2>Latest turn</h2>${d.turns.length?turnCard(d.turns[0]):'<div class="card empty">no turns yet — talk to Milli first</div>'}`;
  },
  loop(d){
  ...
  graph(d){
    ...
    if (!g.enabled)
      h += `<div class="card"><b>Off</b> — every turn currently runs the classic loop.
        ...
        <code>MILLI_GRAPH_WORKFLOWS=1</code> in <code>.env</code>. Any failure anywhere fails open to the
  ...
  settings(d){
    ...
    h += `<h2>Provider &amp; keys (BYOK)</h2><div class="card">
      ...
      ${st.base_url?`<div class="meta" style="margin:4px 0 8px">Custom endpoint active: <code>${esc(st.base_url)}</code> (MILLI_BASE_URL${st.custom_key_set?" + MILLI_API_KEY":""}). The model list below comes from it.</div>`:""}
  ...
  tools(d, sub){
    ...
    h += `<div class="meta" style="margin-bottom:12px">The capabilities the agent can call this turn.
      ...
      ${t.apple_on?"":"Apple tools are off (set <code>MILLI_APPLE_TOOLS=1</code>). "}Connect more via
    ...
    if ((t.planned||[]).length){
      h += `<h2>Coming soon <span class="meta" style="font-weight:400">· on the architecture chart, not wired in yet (opt in with <code>MILLI_EXPERIMENTAL=1</code>)</span></h2>`;
  ...
  database(d, sub){
    ...
    h += `<div class="card" style="border-color:var(--accent);background:var(--accent-soft)">
      ...
      uses a <code>MEMORY.md</code> file, Milli uses these queryable tables — and mirrors them to a
    ...
    h += `<div class="card">
      ...
      <div class="meta">${kb} KB on disk · SQLite + FTS5 · open it yourself: <code>sqlite3 .milli/state.db</code></div>
      <div class="meta" style="margin-top:8px">${reveal("state.db","reveal state.db in Finder")} &nbsp;·&nbsp; ${reveal("","open the .milli folder")}</div></div>`;
  ...
  ops(d){
    ...
    h += `<div class="card"><span class="r">Every LLM call's tokens are logged to
      <code>.milli/usage.jsonl</code> (append-only, never wiped). Dollar cost is estimated from tokens
    ...
    h += (d.trace_tail||[]).length ? table(["event","detail","when"], d.trace_tail.map(e =>
      ...
      : `<div class="card empty">no trace lines yet — talk to Milli</div>`;
