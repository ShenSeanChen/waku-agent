// milli dashboard — formatters + chat card renderers + chatlog + streaming + send.
...
function histItem(m){
  ...
  if (m.meta) return {role:"milli", reply:m.content, gate:m.meta.gate,
                      graph:m.meta.graph,
                      tools:m.meta.tools, iterations:m.meta.iterations,
                      latency_ms:m.meta.latency_ms, model:m.meta.model};
  return {role:"milli", reply:m.content, historical:true};
}
...
const streamingCard = m => `<div class="card">
  ...
  ${m.stream
     ...
     : `<div class="meta" style="margin:0">thinking&hellip;${m.started?` ${Math.round((Date.now()-m.started)/1000)}s`:""}${
         ...
         ? `<br>still waiting: slow models (free tiers especially) can queue for a while; this errors out at the MILLI_LLM_TIMEOUT limit instead of hanging forever`
         : ""}</div>`}
</div>`;
...
function renderChatLog(){
  if (!CHAT.length)
    return `<div class="empty" style="padding:6px 2px">Message Milli here from any tab. Open Overview to watch it flow through the harness, or the Gateway tab to see every channel's messages together.</div>`;
...
async function sendChat(fromInput){
  ...
  const pending = {role:"milli", pending:true, stream:"", started: Date.now()};
  CHAT.push(pending);
  syncChatLogs();
