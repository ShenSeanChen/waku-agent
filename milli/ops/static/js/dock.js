// milli dashboard — chat sessions/history (loadThreadInto), model chip, stats toggle.
...
function applyTele(){
  const off = localStorage.getItem("milli_tele") === "0";
...
function toggleTele(){
  const off = localStorage.getItem("milli_tele") === "0";
  localStorage.setItem("milli_tele", off ? "1" : "0");   // flip
  applyTele();
}
