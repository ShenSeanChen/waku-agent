"""`python -m milli gather` — the morning briefing, run as a graph.

    30 7 * * *  cd ~/milli-agent && make gather

This is the ONE place where real callables meet the pure workflow in
milli/graph/workflows/gather.py. Both the CLI and the dashboard's
...
from milli.graph import run_graph
from milli.graph.workflows.gather import DIGEST_PROMPT, build_gather_graph

DEFAULT_TOPICS = "AI agent harness loop memory eval"


def _github(settings) -> dict:
    """Open PRs and issues. Calls milli/tools/github.py as a LIBRARY, not through
    the registry — so a gather works with MILLI_GH_TOOL off. That flag decides
...
def _web(settings) -> str:
    from milli.tools import search
...
def _calendar(settings) -> dict:
    ...
    from milli.graph.workflows.triage import todays_events
...
def _memory(settings) -> str:
    ...
    Opens its OWN sqlite connection rather than reusing milli.memory's. A
...
def _synthesize(milli, state: dict) -> str:
    ...
    resp = milli.client.messages.create(
        model=milli.settings.model, max_tokens=1500,
...
def build_bound_graph(milli: Milli):
    ...
    return build_gather_graph(
        ...
        synth_fn=lambda state: _synthesize(milli, state),
        draft_fn=lambda state: _draft(s.home, state),
    )


def run_gather(milli: Milli | None = None, observer=None) -> dict:
    ...
    """
    own = milli is None
    milli = milli or Milli()
    try:
        def notify(kind: str, ev: dict) -> None:
            milli.tracer.event(kind, ev)
            if observer:
                observer(kind, ev)

        return run_graph(build_bound_graph(milli), {}, observer=notify)
    finally:
        if own:
            milli.close()
