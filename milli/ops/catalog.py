...
2. **What you chose.** `.milli/models.json` holds an ordered `provider:model`
...
from milli.ops.pricing import remember_price
...
def list_models(provider: str | None = None) -> dict:
    ...
    OpenAI-compatible endpoints (OpenRouter, Gemini, any MILLI_BASE_URL), or the
    ...
    # MILLI_BASE_URL only applies to the provider it was set for).
    ...
    req = urllib.request.Request(url, headers={
        ...
        "User-Agent": "Mozilla/5.0 (compatible; Milli)",
    })
    try:
...
def pinned_specs() -> list[str]:
    ...
    .milli/models.json. The chat switcher shows exactly these. Before they've
