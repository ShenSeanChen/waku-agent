...
from milli.memory import consolidation, retrieval_gate
...
def bundled_skill_dirs() -> list[Path]:
    """Where the skills that SHIP with Milli live — and why there are two answers.
    ...
    the wheel only packages the `milli/` directory, so a `pip install milli-agent`
    ...
    `milli/skills`, so an installed Milli finds it next to the code.
...
class Memory:
    ...
    def _make_fact_store(conn, settings):
        if settings.semantic_store == "supabase":
            from milli.memory.semantic.supabase_store import SupabaseFactStore
    ...
    def export_markdown(self) -> None:
        ...
        whiteboard's `~/.milli/MEMORY.md` box is literally real, and "your memory
        ...
        lines = [
            "# Milli memory",
            "",
            ("_A human-readable mirror of what Milli remembers. The source of truth is "
