"""Skill installer — try anyone's skill in one command.
    ...
    python -m milli skill install https://github.com/<user>/<repo>/blob/main/skills/foo/SKILL.md
    python -m milli skill install https://gist.github.com/<user>/<id>

Downloads the SKILL.md, validates the frontmatter (same check CI runs on
community PRs), and drops it in MILLI_HOME/skills/<name>/ where the loader
...
from milli.memory.procedural.loader import _parse
...
def install(url: str) -> None:
    ...
    print("It loads next time Milli starts. Read it first — skills are instructions.")
