...
only moves words in and out (that's the whole point of the gateway box).
  ...
  voice  macOS `say` with a British voice by default (zero setup), or the
         ...
         then set MILLI_TTS=kokoro  (MILLI_VOICE=bm_george / bm_fable / ...)
...
from milli.app import Milli
from milli.gateway.cli import _observer  # show gate/tool lines in voice mode too
...
class Ears:
    def __init__(self, model_size: str | None = None):
        ...
        self.model = WhisperModel(
            model_size or os.getenv("MILLI_WHISPER_MODEL", "base"),
            compute_type="int8",
        )

    def transcribe(self, audio, language: str | None = None) -> str:
        segments, _ = self.model.transcribe(
            audio, language=language or os.getenv("MILLI_WHISPER_LANG")
        )
...
class Mouth:
    ...
    def __init__(self):
        self.engine = os.getenv("MILLI_TTS", "").strip().lower()
        self.voice = os.getenv("MILLI_VOICE", "")
        if not self.engine:
    ...
    def speak(self, text: str) -> None:
        ...
        if self.engine == "kokoro":
            ...
            print("(no TTS engine on this platform — set MILLI_TTS=kokoro)")


def matches_wake(text: str, wake_word: str) -> bool:
    """Does a transcript contain the (customizable) wake word?

    Fuzzy on purpose: Whisper hears "milli" as "millie", "milly", "silly milli",
    or mangles it entirely. So: `wake_word` accepts comma-separated variants
    across scripts (e.g. "milli,米莉" for a CJK household), normalization keeps
    kana AND CJK, and matching is substring + sliding-window similarity.
    Pure function → deterministic evals.
    """
    import difflib
    import re
...
def _mic_threshold() -> float:
    ...
    MILLI_MIC_THRESHOLD (lower if it never hears you, higher if it
    wakes on room noise)."""
    return float(os.getenv("MILLI_MIC_THRESHOLD", "0.005"))
...
def wake_loop(milli: Milli, mouth: Mouth, wake_word: str) -> None:
    ...
    - The mic buffer is drained after Milli speaks, so it doesn't wake on
    ...
    ack = os.getenv("MILLI_WAKE_ACK", "Yes?")
    followup = float(os.getenv("MILLI_FOLLOWUP_SECONDS", "8"))  # stay open, Siri-style
    block = SAMPLE_RATE // 10
    # Pin the scout's transcription language to match the wake word's script —
    # otherwise Whisper may guess the wrong script for an ambiguous phrase, and
    # a latin wake word never matches CJK output. Commands after wake still auto-detect.
    wake_lang = os.getenv("MILLI_WAKE_LANG") or ("en" if wake_word.isascii() else None)
        ...
        while True:
            ...
            if not matches_wake(heard_scan, wake_word):
                ...
                if heard_scan:  # near-misses belong in the trace (wake tuning!)
                    milli.tracer.event("wake_scan", {"heard": heard_scan, "matched": False})
                continue
            ...
            # for a follow-up for `followup` seconds — no need to say "milli"
            # again (like Siri). A quiet stretch drops back to wake-word mode.
            while True:
                heard = ears.transcribe(record_command(stream))
                if heard:
                    print(f"you › {heard}")
                    result = milli.respond(heard, observer=_observer, source="voice")
                    print(f"milli › {result.reply}")
...
def main() -> None:
    ...
    milli = Milli()
    milli.session.session_id = "voice"   # its own conversation thread in the inbox
    mouth = Mouth()

    # Hands-free by default: always-listening for "milli". The default packs in
    # the ways the tiny scanner mis-hears it (millie / milly / hey milli), so
    # it triggers reliably. Set MILLI_WAKE_WORD="" for push-to-talk instead.
    wake_word = os.getenv(
        "MILLI_WAKE_WORD", "milli,millie,milly,hey milli"
    ).strip()
    if wake_word:
        try:
            wake_loop(milli, mouth, wake_word)
    ...
    ears = Ears()
    print("Voice Milli ready. Press Enter to talk, Ctrl-C to quit.")
    while True:
        try:
            input("\npress Enter to talk… ")
