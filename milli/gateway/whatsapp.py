...
Setup (10-20 minutes, free tier available but painful):
  ...
  5. Set up webhook verification (WHATSAPP_VERIFY_TOKEN):
     - Pick any random string, e.g. "my-milli-verify-token-123"
...
def _build_handler(
    ...
    s = load_settings()
    s.ensure_home()
    milli = Milli(settings=s, conn=connect(s.home, check_same_thread=False))
    milli.session.session_id = "whatsapp"
    milli_lock = threading.Lock()
...
def main() -> None:
    try:
        ...
        raise SystemExit("WhatsApp extra not installed: pip install 'milli-agent[whatsapp]'")
    ...
    if not token:
        raise SystemExit(
            ...
            "See milli/gateway/whatsapp.py docstring for setup instructions."
        )
...
def start_in_background() -> bool:
    ...
    `milli dashboard` runs the browser cockpit AND WhatsApp from one command.
    ...
    try:
        ...
        print("(whatsapp) WHATSAPP_TOKEN is set but the extra isn't installed — "
              "pip install 'milli-agent[whatsapp]'")
        return False
