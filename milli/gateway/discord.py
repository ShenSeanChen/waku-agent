"""Discord gateway — message Milli from a direct message or a server channel.

READ THIS BEFORE PUTTING THE BOT IN A SHARED SERVER.

This gateway runs YOUR agent: your memory (`.milli/state.db`), your SOUL.md, your
...
Setup:
  ...
  4. Open it up only as far as you actually need (all optional):
     ...
     DISCORD_HOME              a SEPARATE .milli for the bot. Use this whenever
...
from milli.gateway.cli import _observer
...
def _strip_mention(content: str, bot_id: str) -> str:
    """Drop the '@Milli Agent' prefix so the model sees the question, not the ping."""
    for form in (f"<@{bot_id}>", f"<@!{bot_id}>"):
        content = content.replace(form, " ")
    return content.strip()


def _build_agent() -> Milli:
    ...
    return Milli(settings=settings)
...
def _build_client():
    ...
    milli = _build_agent()
    milli.session.session_id = "discord"
    ...
    async def on_message(message: discord.Message) -> None:
        ...
        async with message.channel.typing():
            result = milli.respond(text, observer=_observer, source="discord")
...
def describe_posture() -> str:
    ...
    home = os.getenv("DISCORD_HOME", "").strip() or ".milli — YOUR personal memory"
...
def main() -> None:
    try:
        ...
        raise SystemExit("Discord extra not installed: pip install 'milli-agent[discord]'")

    token = os.getenv("DISCORD_BOT_TOKEN", "")
    if not token:
        raise SystemExit("Set DISCORD_BOT_TOKEN in .env (create a bot in the Discord Developer Portal).")
    client = _build_client()
    print("Milli is listening on Discord. Ctrl-C to stop.")
...
def start_in_background() -> bool:
    ...
    try:
        ...
        print("(discord) DISCORD_BOT_TOKEN is set but the extra isn't installed — "
              "pip install 'milli-agent[discord]'")
        return False
