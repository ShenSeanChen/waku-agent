...
away. What persists lives in milli/memory. Working memory =

    system prompt (SOUL.md)            ← who Milli is
  + durable facts & episodes           ← what Milli remembers (gated!)
...
DEFAULT_SOUL = """\
You are Milli, a personal assistant running locally on your user's laptop.
...
- Be honest about where things live. Every tool's output states exactly where
  ...
  .milli/state.db) — relay that truthfully, and never claim something synced
...
def load_soul(settings: Settings) -> str:
    ...
    changes who your Milli is — that's procedural memory at its simplest."""
...
class Session:
    ...
    def __init__(self, settings: Settings, memory=None, session_id: str = "default"):
        ...
        self.memory = memory  # milli.memory.Memory (None until Phase-2 wiring)
    ...
    def build_system(self, user_message: str, notify=None) -> str:
        ...
        parts = [load_soul(self.settings),
                 ...
                 f"'{self.settings.provider}' provider, inside Milli, a local-first "
