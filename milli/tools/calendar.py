...
Where events land:
  ...
  opt-in      Apple Calendar, in a dedicated "Milli" calendar, via AppleScript —
              set MILLI_APPLE_CALENDAR=1. First use makes macOS ask permission
              for your terminal to control Calendar; approve once.
  opt-in      Google Calendar via MILLI_GOOGLE_CALENDAR=1. Local files remain
...
relays it, so Milli never over-claims what happened.
"""
...
from milli.tools.registry import Tool

APPLE_CALENDAR_NAME = "Milli"
...
def sync_to_apple_calendar(title: str, start: str, end: str, notes: str = "") -> str:
    """Create the event in Calendar.app under the 'Milli' calendar (created on
    ...
    # Prefer a dedicated "Milli" calendar, but macOS can't create calendars in
...
def make_tool(
    ...
    def create_event(
        ...
        if not apple_calendar and not google_calendar:
            where += (
                " Not synced to any calendar app (enable with MILLI_APPLE_CALENDAR=1 "
                "or MILLI_GOOGLE_CALENDAR=1, "
...
def make_list_tool(conn: sqlite3.Connection, home: Path | None = None) -> Tool:
    ...
    milli itself created. Every source is LABELLED in the output, so the agent can
    ...
    def list_events(start: str = "", end: str = "", limit: int = 20) -> str:
        ...
        if home is not None:
            from milli.tools.google_calendar import is_connected, list_google_events
        ...
        checked.append("milli's local calendar")
        local = local_events(start, end, limit)
        if local:
            sections.append("From milli's local calendar (events milli created):\n" + local)
    ...
    return Tool(
        ...
        description=(
            ...
            "when signed in, plus milli's own local calendar). "
