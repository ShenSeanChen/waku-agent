...
Two doors, deliberately different:
  ...
  READ   (calendar.events.readonly)  milli's own OAuth client, shipped below.
         Click, approve in the browser, done. No files to download.

  WRITE  (calendar.events)           your own `.milli/credentials.json`.
         Creating events on someone's calendar is a sensitive scope, so milli
...
# milli's own OAuth client (Desktop app). Fill these in from the Google Cloud
...
_SETUP_HINT = (
    "Google Calendar is not connected yet. Run `milli connect google` (or click "
...
def connect(home: Path) -> str:
    ...
    Uses milli's shipped client for read-only access. If you dropped your own
    `.milli/credentials.json` in place, that wins and you get the write scope
    too — bring-your-own-client is the escape hatch for people who want milli
    creating events."""
    try:
    ...
    try:
        if own.exists():
            ...
            return (
                "Google Calendar has no client configured. Either wait for a milli "
                "release with one built in, or create your own OAuth client "
                "(Desktop app) and save it as .milli/credentials.json."
            )
