...
Apple ecosystem (MILLI_APPLE_TOOLS=1) and MCP servers (.milli/mcp.json)."""
...
from milli.tools.registry import ToolRegistry
...
def build_registry(conn: sqlite3.Connection, settings: Settings, memory=None) -> ToolRegistry:
    ...
    # source (Google when signed in, plus milli's own), so the model never has
    ...
    # Experimental tools — off by default; opt in with MILLI_EXPERIMENTAL=1.
    ...
    # MILLI_EXPERIMENTAL, so re-checking the env here would let the global switch
    # override an explicit False — and the arena passes experimental=False for
    # every non-coding race. Once the dashboard could write MILLI_EXPERIMENTAL=1,
    # that OR silently forced delegate_task into races that never asked for it.
    if getattr(settings, "experimental", False):
        from milli.tools import experimental
    ...
    # MCP servers (opt-in via .milli/mcp.json).
    mcp_config = settings.home / "mcp.json"
    if mcp_config.exists():
        try:
            from milli.tools.mcp_client import MCPBridge

            bridge = MCPBridge(mcp_config)
            for t in bridge.start():
                registry.register(t)
            registry.mcp_bridge = bridge  # so Milli.close() can stop the servers
        except ImportError:
            print("mcp.json found but the 'mcp' package is missing — pip install 'milli-agent[mcp]'")
