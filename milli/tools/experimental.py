...
The division of labor is the teaching point: Milli is the orchestrator (memory,
working-memory assembly, evals, the human's context) and pi is the specialist
contractor (read/bash/edit/write, pure coding craft). Milli hires; pi codes;
Milli's release gate can then inspect the work.
...
`MILLI_EXPERIMENTAL=1` to register these tools.
"""
...
def _project_pi_flags() -> list[str]:
    ...
    This is the "pi x milli" payoff: milli upgrades its coding contractor without
    touching pi's source. pi auto-discovers project resources from cwd upward,
    but delegated runs happen in milli_workspace/ (outside the repo), so we pass
...
def make_delegate_tool(settings: Settings) -> Tool:
    ...
    Same honesty contract as every Milli tool — the return string says exactly
    ...
    def delegate_task(task: str = "", cwd: str = "", timeout_seconds: int = 0,
        ...
        from milli.tools import workspace
        if cwd:
        ...
        timeout = int(timeout_seconds) or int(os.getenv("MILLI_DELEGATE_TIMEOUT", "300"))
        ...
        from milli.ops.coding_eval import PI_PROVIDER, _key_for
        cmd = [pi_bin]
        ...
        if json_mode:
            ...
            if code is None:
                return (f"pi was still working after {timeout}s so I stopped it — try a smaller "
                        f"task, or raise MILLI_DELEGATE_TIMEOUT.")
            stdout_text = reply
        else:
            try:
...
def make_tools(settings: Settings) -> list[Tool]:
    """Experimental tools, registered only when MILLI_EXPERIMENTAL=1: the live
