...
Pick with MILLI_PROVIDER=anthropic|openai|gemini|deepseek|minimax|kimi|glm|openrouter
and set that provider's API key in .env. Override the model ids with MILLI_MODEL /
MILLI_SMALL_MODEL if the defaults below age out — they're just strings. This
matters most for openrouter: it's a single key in front of hundreds of models,
so MILLI_MODEL=<vendor>/<model> (e.g. "google/gemini-3.5-flash") picks whichever
...
PROVIDERS: dict[str, Provider] = {
    ...
    # /v1/chat/completions (they need /v1/responses), so every Milli turn 400s on
    ...
    # checked). Override with MILLI_SMALL_MODEL=kimi-k3 if your key is K3-only.
...
def get_client(settings: Settings):
    ...
    if provider is None:
        raise SystemExit(f"Unknown MILLI_PROVIDER '{settings.provider}'. "
    ...
    timeout = float(os.getenv("MILLI_LLM_TIMEOUT", "120"))
