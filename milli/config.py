...
file, you know everything Milli can be configured to do.
"""
...
@dataclass
class Settings:
    # --- LLM: pick a provider, set its key. See milli/loop/models.py PROVIDERS.
    provider: str = field(default_factory=lambda: os.getenv("MILLI_PROVIDER", "anthropic"))
    ...
    api_key: str = field(default_factory=lambda: os.getenv("MILLI_API_KEY", ""))
    base_url: str | None = field(default_factory=lambda: os.getenv("MILLI_BASE_URL") or None)
    model: str = field(default_factory=lambda: os.getenv("MILLI_MODEL", ""))
    # Cheap model used by the retrieval gate and the consolidation summarizer.
    small_model: str = field(default_factory=lambda: os.getenv("MILLI_SMALL_MODEL", ""))

    # --- Home: where Milli keeps its state (memory DB, calendar, outbox, traces).
    # Defaults to ./.milli next to where you run it, so you can open every file
    # it writes. Local-first means you can always look.
    home: Path = field(default_factory=lambda: Path(os.getenv("MILLI_HOME", ".milli")))

    # --- Loop guardrails
    max_iterations: int = field(default_factory=lambda: int(os.getenv("MILLI_MAX_ITERATIONS", "10")))
    ...
    max_tokens: int = field(default_factory=lambda: int(os.getenv("MILLI_MAX_TOKENS", "8192")))
    ...
    history_turns: int = field(default_factory=lambda: int(os.getenv("MILLI_HISTORY_TURNS", "12")))

    # --- Memory
    # Consolidate (distill chats into durable facts) only after N new exchanges.
    consolidate_every: int = field(default_factory=lambda: int(os.getenv("MILLI_CONSOLIDATE_EVERY", "6")))
    retrieval_top_k: int = field(default_factory=lambda: int(os.getenv("MILLI_RETRIEVAL_TOP_K", "4")))
    # 'sqlite' (default, zero setup) or 'supabase' (pgvector upgrade path — see launch-rag).
    semantic_store: str = field(default_factory=lambda: os.getenv("MILLI_SEMANTIC_STORE", "sqlite"))
    # 'sqlite' (default, zero setup) or 'notion' (episodes live in a Notion database).
    episodic_store: str = field(default_factory=lambda: os.getenv("MILLI_EPISODIC_STORE", "sqlite"))

    # --- Tools
    # Sync created events into Apple Calendar (a dedicated "Milli" calendar)
    # via AppleScript. Opt-in because it writes to your real calendar app.
    apple_calendar: bool = field(
        default_factory=lambda: os.getenv("MILLI_APPLE_CALENDAR", "") in ("1", "true", "yes")
    )
    ...
    google_calendar: bool = field(
        default_factory=lambda: os.getenv("MILLI_GOOGLE_CALENDAR", "") in ("1", "true", "yes")
    )
    google_calendar_id: str = field(
        default_factory=lambda: os.getenv("MILLI_GOOGLE_CALENDAR_ID", "") or "primary"
    )
    ...
    apple_tools: bool = field(
        default_factory=lambda: os.getenv("MILLI_APPLE_TOOLS", "") in ("1", "true", "yes")
    )
    ...
    # The gather workflow calls milli/tools/github.py as a library and does NOT
    # need this on — the switch only decides whether the MODEL can reach it.
    gh_tool: bool = field(
        default_factory=lambda: os.getenv("MILLI_GH_TOOL", "") in ("1", "true", "yes")
    )
    # owner/name to assume when a call omits it — for when Milli runs outside a
    # checkout, where `gh` has no remote to infer from.
    gh_repo: str = field(default_factory=lambda: os.getenv("MILLI_GH_REPO", ""))
    ...
    experimental: bool = field(
        default_factory=lambda: os.getenv("MILLI_EXPERIMENTAL", "") in ("1", "true", "yes")
    )
    ...
    # the plain loop, so this can never make Milli worse — only faster/cheaper.
    graph_workflows: bool = field(
        default_factory=lambda: os.getenv("MILLI_GRAPH_WORKFLOWS", "") in ("1", "true", "yes")
    )
