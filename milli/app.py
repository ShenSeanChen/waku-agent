"""Wiring — builds one Milli from its parts. Gateways call `respond()`.
...
from milli.loop.agent import LoopResult, Observer, run_loop
from milli.loop.models import get_client
from milli.ops.tracing import Tracer, compose
from milli.runtime.session import Session
...
class Milli:
    ...
    def respond(self, user_message: str, observer: Observer | None = None,
        ...
        with self.tracer.turn(user_message):
            # The graph front door is optional and can NEVER make Milli worse:
    ...
    def _respond_via_graph(self, user_message: str, notify, stream: bool) -> LoopResult | None:
        ...
        from milli.graph.workflows.triage import (
            QUICK_REPLY_PROMPT,
            build_triage_graph,
            classify_message,
            todays_events,
